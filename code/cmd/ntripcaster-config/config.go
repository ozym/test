...
func (c *Config) Sort() {
	sort.Slice(c.Users, func(i, j int) bool { return c.Users[i].Less(c.Users[j]) })
	sort.Slice(c.Groups, func(i, j int) bool { return c.Groups[i].Less(c.Groups[j]) })
	sort.Slice(c.ClientMounts, func(i, j int) bool { return c.ClientMounts[i].Less(c.ClientMounts[j]) })
	sort.Slice(c.Mounts, func(i, j int) bool { return c.Mounts[i].Less(c.Mounts[j]) })
}
